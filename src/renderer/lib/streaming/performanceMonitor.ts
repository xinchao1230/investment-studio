 ...
 * Monitors typewriter effect performance metrics and provides optimization recommendations
 */
...
export class StreamingPerformanceMonitor {
  ...
  startMonitoring(): void {
    ...
    logger.debug('[PerformanceMonitor] Starting performance monitoring');
  }
  ...
  stopMonitoring(): PerformanceMetrics {
    ...
    logger.debug('[PerformanceMonitor] Monitoring stopped, final metrics:', metrics);
  ...
  getMetrics(): PerformanceMetrics {
    ...
    const recentFrameTimes = this.frameTimes.slice(-30); // most recent 30 frames
  ...
  private addAlert(alert: PerformanceAlert): void {
    ...
    // Log according to severity level
...
`;
  ...
  private getPerformanceRecommendations(metrics: PerformanceMetrics, score: number): string {
    ...
    if (score >= 90) {
      recommendations += '  ✅ Performance is excellent, no optimization needed\n';
      return recommendations;
    }

    if (metrics.averageFPS < 45) {
      recommendations += '  • Frame rate is low; consider enabling incremental rendering\n';
    }

    if (metrics.averageRenderTime > 20) {
      recommendations += '  • Render time is too long; consider reducing batch size\n';
    }

    if (metrics.memoryDeltaMB > 30) {
      recommendations += '  • Memory growth is large; consider clearing old render caches\n';
    }

    if (metrics.charactersPerSecond < 30) {
      recommendations += '  • Typing speed is slow; consider increasing batch size\n';
    }
