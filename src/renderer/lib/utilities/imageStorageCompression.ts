 ...
 * Specifically for compressing images into history storage after a conversation ends
 */
 ...
 * Goal: ≤100KB to reduce context overhead
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Optimize for storage during drawing
        ...
        const tryCompress = () => {
          ...
          // Calculate actual file size (decoded bytes from base64)
...
/**
 * Extract and compress an image from an ImageContentPart
 */
  ...
  try {
    ...
    const compressedImagePart: ImageContentPart = {
      ...
      metadata: {
        ...
        // New field to mark compression status
...
export async function compressMessageImagesForStorage(
  ...
  for (let i = 0; i < message.content.length; i++) {
    ...
    if (part.type === 'image') {
      ...
      // Check whether storage compression is needed
...
/**
 * Check whether an image needs storage compression
 */
