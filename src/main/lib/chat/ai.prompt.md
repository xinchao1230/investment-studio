...
- ⚠️ The `AgentChat` token counter's tiktoken encoding must be provider-aware. `getCurrentModelConfig`/`getModelCapabilities` already branch on `providerManager.getActiveProviderId()`, but the token counter's own encoding (constructor + `getTokenCounter()` recreate path) is resolved via `resolveTokenizerEncoding(modelId)`: for non-Copilot providers it reads `PROVIDER_TOKENIZER[activeProviderId]`, NOT the module-level `getModelCapabilities` (which only knows Copilot models and would return null → `o200k_base`, an UNDER-count for Anthropic/Gemini that defeats the conservative-overestimate design — see token-overflow postmortem). Do not revert these two call sites to the bare `getModelCapabilities(modelId)?.tokenizer` lookup.
- ⚠️ Compression runs as a secondary LLM call inside an active chat turn, using `AgentChat.getCurrentModelId()` (the user's currently selected model). Auth-token / provider failures during compression fall back to truncation — the user sees no error, but conversation context may be degraded. Compression must never assume a specific hardcoded model is available; pass the active chat's model through `compressContextHistoryWithFullMode(history, compressor, agentName, modelId, encoding?)`. The optional `encoding` argument should be `PROVIDER_TOKENIZER[providerManager.getActiveProviderId()]` so chunk budgeting uses the right tiktoken family (cl100k_base for Anthropic/Gemini/custom-dynamic, o200k_base for Copilot/OpenAI); omitting it falls back to the compressor's `cl100k_base` default which is a safer overestimate but still wrong for OpenAI-family models.
