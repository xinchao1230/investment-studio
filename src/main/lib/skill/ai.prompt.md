...
| `installAndActivateSkill.ts` | `installAndActivateSkill()` — unified entry point: installs from device path, then applies to agents per activation mode (`current-agent`, `all-agents`, `install-only`, etc.) | medium |
