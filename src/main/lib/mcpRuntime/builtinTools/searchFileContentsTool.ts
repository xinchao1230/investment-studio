/**
 * SearchFileContentsTool - Built-in file content text search tool (literal / regex)
 * Core functionality: search text content inside files; does NOT search file names or paths
 * Key features:
 *  - workspaceRoot (required) for global scanning or resolving relative paths
 *  - Supports literal (case-insensitive) and /regex/ patterns (auto-adds the i flag)
 *  - Simple glob filtering: *, *.ext, **\/*.ext (no complex wildcard parsing)
 *  - Context lines 0–2; returns aggregated "match blocks" (hit lines prefixed with '>')
...
export interface SearchFileContentsToolArgs {
...
export interface SearchFileContentsMatchBlock {
...
export interface SearchFileContentsFileResult {
  file: string;                  // Relative to root or absolute (if no root)
  matches: SearchFileContentsMatchBlock[];
}

export interface SearchFileContentsPatternResult {
  pattern: string;               // The pattern currently being executed
  results: SearchFileContentsFileResult[]; // List of files with matches
...
export interface SearchFileContentsToolResult {
  ...
  patternResults: SearchFileContentsPatternResult[]; // Search results divided by pattern
...
export class SearchFileContentsTool {

  /**
   * Execute the file search tool
   * Static method, supports direct LLM invocation
   */
  static async execute(args: SearchFileContentsToolArgs, options?: { signal?: AbortSignal }): Promise<SearchFileContentsToolResult> {

    // 1. Validate arguments
    // Validate basic argument types and required fields
    ...
    try {
      ...
      const patternResults: SearchFileContentsPatternResult[] = [];
      ...
      for (const pattern of normalizedPatterns) {
        ...
        const patternResultsForFiles: SearchFileContentsFileResult[] = [];
        let totalMatches = 0;
        let filesScanned = 0;
      ...
      const output: SearchFileContentsToolResult = {
        success: true,
      ...
      throw new Error(`search_file_contents execution failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private static async resolveTargets(args: SearchFileContentsToolArgs, workspaceRoot: string | null, errors: string[]): Promise<Array<{abs: string; rel: string; isFile: boolean}>> {
  ...
  private static async processFile(abs: string, rel: string, workspaceRoot: string | null, ctx: any): Promise<void> {
    ...
    const blocks: SearchFileContentsMatchBlock[] = [];
    let current: number[] = [];
   ...
   */
  private static buildBlock(lineNumbers: number[], allLines: string[], context: number): SearchFileContentsMatchBlock {
   ...
   */
  private static validateArgs(args: SearchFileContentsToolArgs): { isValid: boolean; error?: string; normalizedPatterns?: string[]; removedEntries?: number } {
