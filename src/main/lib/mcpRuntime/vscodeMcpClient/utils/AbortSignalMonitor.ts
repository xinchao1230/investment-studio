...
export class AbortSignalMonitor {
  ...
  static addListener(
    ...
    if (recentSources.length > 5) { // Allow up to 5 similar sources
      this.logActivity(`⚠️ Skipping duplicate source listener: ${source} (already have ${recentSources.length} similar)`);
      return;
    }
    ...
    setTimeout(() => {
      if (!signal.aborted) {
        this.decrementListener(signal, uniqueKey);
        this.logActivity(`Force-cleaning timed-out listener: ${uniqueKey}`);
      }
  ...
  static checkForLeaks(): {
    ...
    if (stats.totalListeners > 5000) { // Increased threshold for multiple servers
      ...
      recommendations.push('Check for listeners that are not being cleaned up properly');
    }

    if (stats.criticalSignals > 0) {
      warnings.push(`${stats.criticalSignals} signal(s) are approaching the listener limit`);
    ...
    if (stats.oldestSignal && stats.oldestSignal.age > 300000) { // 5 minutes
      warnings.push(`Long-lived signal detected: ${Math.round(stats.oldestSignal.age / 60000)} minutes`);
      recommendations.push('Review signal lifecycle management');
    }
        ...
        if (type === 'abort' && AbortSignalMonitor.isEnabled) {
          ...
          if (!isFromMonitor) {
            ...
            setTimeout(() => {
              if (!this.aborted) {
                AbortSignalMonitor.decrementListener(this, cleanupKey);
                AbortSignalMonitor.logActivity(`Force-cleaning timed-out global-intercepted listener: ${cleanupKey}`);
              }
