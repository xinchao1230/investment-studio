...
export class ResearchMcpInstallManager extends EventEmitter {
  ...
  private async doInstall(): Promise<{ ok: boolean; error?: string }> {
    // Stage 1: detect uv
    this.emit('progress', { stage: 'detect_uv', percent: 5, message: '检测 uv 工具...' } as InstallProgress);
    if (!fs.existsSync(this.uvPath)) {
      return { ok: false, error: `uv not found at ${this.uvPath}` };
    }
    this.emit('progress', { stage: 'detect_uv', percent: 15, message: 'uv 已就绪' } as InstallProgress);
    if (this.cancelled) return this.cleanupCancel();

    // Stage 2: create venv
    this.emit('progress', { stage: 'create_venv', percent: 20, message: '创建 Python 虚拟环境...' } as InstallProgress);
    fs.mkdirSync(this.runtimeDir, { recursive: true });
    const venvPath = path.join(this.runtimeDir, '.venv');
    await this.run(this.uvPath, ['venv', venvPath, '--python', '3.11']);
    this.emit('progress', { stage: 'create_venv', percent: 35, message: '虚拟环境创建完成' } as InstallProgress);
    if (this.cancelled) return this.cleanupCancel();

    // Stage 3: install deps
    this.emit('progress', { stage: 'install_deps', percent: 40, message: '安装依赖包...' } as InstallProgress);
    ...
    this.emit('progress', { stage: 'install_deps', percent: 85, message: '依赖安装完成' } as InstallProgress);
    if (this.cancelled) return this.cleanupCancel();

    // Stage 4: health check + write meta
    this.emit('progress', { stage: 'health_check', percent: 90, message: '完成校验...' } as InstallProgress);
    ...
    this.emit('progress', { stage: 'health_check', percent: 100, message: '安装完成' } as InstallProgress);
    return { ok: true };
  }
