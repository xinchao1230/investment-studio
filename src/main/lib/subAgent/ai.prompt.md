...
- `context_access` is applied at spawn time by `SubAgentManager`: `isolated`/missing injects no parent conversation text, `parent_summary` injects the live parent chat summary, and `full_history` injects parent history unless it exceeds 50% of a 128K window, then falls back to the summary. All inherited parent text is wrapped in reference-only `<parent_context>` tags by `sanitizeContextForSubAgent()` before `SubAgentChat` adds it to the system prompt.
