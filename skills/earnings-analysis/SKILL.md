...
description: "Create professional equity research earnings update reports analyzing quarterly results. Triggers on: earnings update, quarterly update, earnings analysis, Q1/Q2/Q3/Q4 results, post-earnings report, 业绩分析报告, 季报深度, 业绩点评报告, 财报点评, 帮我看看财报, 分析最新财报"
---
...
## Data Sources & Cache Policy

Follow the 3-tier data source priority:
1. **Institutional MCP** (FactSet, Daloopa, S&P Kensho) — if configured, use exclusively
2. **research-mcp** (tushare_collect / yfinance_collect) — default path for A-share and US/HK stocks
3. **Web search** — fallback for consensus estimates, earnings call transcripts, news

### Cache Check (MANDATORY before any data fetch)

Before any `*_collect` call, follow `skills/_cache-policy.md`:
- Read `{target_dir}/data-cache/tushare/{endpoint}.meta.json` for each endpoint (income / balancesheet / cashflow). Within TTL (income/balancesheet/cashflow = 7d) → reuse the CSV, skip the collect call.
...
- Force refresh when the user says "latest data / refresh / 最新数据 / 刷新 / 重新拉取".
...
Use `yfinance_collect` for US and HK listed companies.

## Portfolio Integration

### Before Analysis

1. Use `portfolio_get_target_files` to check if the target exists in the portfolio.
2. If exists, read `key-drivers.md` to understand what metrics matter for this company.
3. If not exists, call `portfolio_init_target` first.

### After Analysis

1. Save the earnings report to: `{target_dir}/earnings/{period}-review.md`
2. Log completion: `portfolio_append_note` with "Earnings analysis completed for {period}"
3. If key drivers changed, update `{target_dir}/key-drivers.md`
