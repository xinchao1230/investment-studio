  ...
  async execute(args: NavigateToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      if (existingTab?.id !== undefined) {
        ...
        // Update URL when it differs from what the tab currently has
        // (covers explicit tabId, hash-routing SPAs, and query changes)
        const currentTabUrl = existingTab.url || '';
        if (typeof existingTab.id === 'number' && url && currentTabUrl !== url) {
