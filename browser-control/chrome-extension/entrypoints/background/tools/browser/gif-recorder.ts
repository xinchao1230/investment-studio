...
import { handleGifExportAction, saveGifToDownloads } from './gif-recorder-export';
  ...
  async execute(args: GifRecorderParams): Promise<ToolResult> {
    ...
    try {
      switch (action) {
        ...
        case 'export': {
          const exportResult = await handleGifExportAction(
            {
              resolveTargetTab: this.resolveTargetTab.bind(this),
              isRestrictedUrl: this.isRestrictedUrl.bind(this),
          ...
          if (exportResult.isError && lastRecordedGif && Date.now() - lastRecordedGif.createdAt > EXPORT_CACHE_LIFETIME_MS) {
            lastRecordedGif = null;
          }
          return exportResult;
        }
